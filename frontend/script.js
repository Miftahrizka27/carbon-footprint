...
function tampilkanHasil() {
    ...
    // Potensi Hemat
    const nilaiKategori = {
      listrik: data.hasil.listrik,
      air: data.hasil.air,
      transportasi: data.hasil.transportasi,
      sampah: data.hasil.sampah
    };
    const kategoriTertinggi = Object.keys(nilaiKategori).reduce((a, b) =>
      nilaiKategori[a] > nilaiKategori[b] ? a : b
    );
    const tipsSaran = {
      listrik: 'beralih ke peralatan hemat energi atau menggunakan panel surya',
      air: 'menghemat konsumsi air harian dan memanfaatkan air hujan',
      transportasi: 'beralih ke transportasi umum atau bersepeda',
      sampah: 'mengurangi sampah makanan dan mulai mendaur ulang'
    };
    const potensiPersen = 15;
    const potensiKg = (data.hasil.total * potensiPersen / 100).toFixed(1);
    document.getElementById('potensi-value').textContent = `~${potensiKg} kg CO₂/bulan`;
    document.getElementById('potensi-desc').textContent =
      `Model kami memprediksi Anda bisa menurunkan jejak karbon hingga ${potensiPersen}% (${potensiKg} kg) jika ${tipsSaran[kategoriTertinggi]}.`;
...
// Clamp number inputs — prevent negatives
['listrik', 'air', 'transportasi', 'sampah'].forEach(id => {
